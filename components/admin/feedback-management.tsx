...
import { MessageSquare, CheckCircle, AlertCircle, Lightbulb, MoreHorizontal } from "lucide-react"
...
export function FeedbackManagement() {
  ...
  const handleStatusChange = (id: number, newStatus: Feedback["status"]) => {
    setLocalFeedbacks((prev) => prev.map((f) => (f.id === id ? { ...f, status: newStatus } : f)))
  }
                ...
                <TableRow key={feedback.id}>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="capitalize text-sm">{feedback.feedback_type}</span>
                    </div>
                  </TableCell>
                  <TableCell className="font-medium">Questão #{feedback.question_id}</TableCell>
                  ...
                  <TableCell className="text-muted-foreground text-sm">{formatDate(feedback.created_at)}</TableCell>
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => handleStatusChange(feedback.id, "implemented")}>
                          Implementado
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
