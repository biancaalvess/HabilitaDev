          ...
          <TableBody>
            {questions.map((question) => (
              <TableRow key={question.id}>
                ...
                <TableCell>
                  <Badge variant="outline" className={`${DIFFICULTY_COLORS[question.difficulty]} text-xs capitalize`}>
                    {question.difficulty}
                  </Badge>
                </TableCell>
                ...
                <TableCell>
                  {question.approved ? (
                    <Badge className="bg-green-500/10 text-green-400 border-green-500/20">Aprovada</Badge>
                  ) : (
                    <Badge className="bg-yellow-500/10 text-yellow-400 border-yellow-500/20">Pendente</Badge>
                  )}
                </TableCell>
                <TableCell className="text-muted-foreground text-sm">{formatDate(question.created_at)}</TableCell>
                <TableCell className="text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuSeparator />
                      <DropdownMenuItem onClick={() => handleDelete(question.id)} className="text-destructive">
