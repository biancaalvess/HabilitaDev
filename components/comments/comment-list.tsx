...
import { MessageSquare, Clock, User, AlertCircle, Lightbulb } from "lucide-react"
              ...
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <Icon className={`h-4 w-4 ${commentTypeColors[comment.comment_type]}`} />
                    <span className="text-sm font-medium">{commentTypeLabels[comment.comment_type]}</span>
                    <Badge variant="outline" className="text-xs">
                      {comment.comment_type === "correction" ? "Correção" : "Sugestão"}
                    </Badge>
                  </div>
                ...
                <p className="text-sm text-foreground leading-relaxed pl-6">{comment.content}</p>
