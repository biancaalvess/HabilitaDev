...
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      // Se o backend estiver indisponível, retornar questão mockada
      if (response.status === 500 || response.status === 502 || response.status === 503) {
        console.log('Backend indisponível, retornando questão mockada');
        const mockQuestion = getMockQuestionById(params.id);
    ...
    console.log('Erro na conexão com backend, retornando questão mockada');
    
    // Em caso de erro de conexão, tentar retornar questão mockada
    const mockQuestion = getMockQuestionById(params.id);
...
// Função para buscar questão mockada por ID
function getMockQuestionById(id: string) {
  ...
  return mockQuestions.find(q => q.id.toString() === id) || null;
}
